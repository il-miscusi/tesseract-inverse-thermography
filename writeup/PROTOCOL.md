...
## Outcome of the v2 run (recorded after the fact; artifacts are the record)

Run on 2026-08-30, 1229 s wall clock (figures/experiment_b_v2.json,
figures/experiment_b_v2_fields.npz, log figures/sweeps/experiment_b_v2_run.log):

- coupled: rel_l2 0.1370, amplitude ratio 1.115, centroid shift 0.02 cells,
  total power ratio 1.000, final data loss 2.022 (noise floor 2.0) — the
  declared gate (rel_l2 < 0.5 AND centroid < 1.5) PASSES.
- frozen-viscosity: rel_l2 0.2457, amplitude ratio 1.098, centroid shift 0.17
  cells, final recorded data loss 3.339 — 1.79x worse in L2 and 65% above the
  noise floor at the matched budget. Both L-BFGS-B arms stopped at the declared
  250-iteration limit (status 1), so this is not presented as a convergence
  floor. The arm changes the forward model and its derivative together.
- Gate G0 re-verified on the final configuration: best relative error
  3.611e-07, PASS (figures/e2e_gradient_check.json).
